  ...
  -> int
{
  ...
  errno = 0;
  if (::bind(_socket_fd, tcp_address.data(), tcp_address.size()) != 0)
  {
    return errno;
  }
  return 0;
...
{
  errno = 0;
  if (::connect(_socket_fd, tcp_address.data(), tcp_address.size()) != 0)
  {
    return errno;
  }
  return 0;
  ...
  -> bool
{
  ...
  while (true)
  {
    ...
    if (remote_address_ptr && *remote_address_ptr)
    {
      ...
      accepted = ::accept4(_socket_fd, remote_address->data(), &address_length, SOCK_NONBLOCK);
