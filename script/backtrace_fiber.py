...
def get_strategy(scheduler):
    strategy_type = gdb.lookup_type('::ustevent::FiberStrategy')
...
def get_debug_infos_in_waiting_fibers(scheduler):
...
def get_debug_infos_in_ready_fibers(scheduler):
    strategy = get_strategy(scheduler)
...
def pretty_stack(backtrace, size):
    ...
    for i in range(size):
        ...
        if start_print:
            print(frame)
            frame_number += 1
        elif 'ustevent::FiberDebugInfo' not in frame and 'ustevent::FiberStrategy' not in frame:
            # skip ustevent::FiberDebugInfo::backtrace()
            #      ustevent::FiberStrategy::_recordBacktrace()
            #      ustevent::FiberStrategy::pick_next()
            start_print = True
            print(frame)
            frame_number += 1
...
def print_waiting_fibers(count, scheduler):
    print('Waiting Fibers:')
    waiting_fiber_debug_infos = get_debug_infos_in_waiting_fibers(scheduler)
...
def print_ready_fibers(count, scheduler):
    print('Ready Fibers:')
    ready_fiber_debug_infos = get_debug_infos_in_ready_fibers(scheduler)
...
class backtrace_fiber(gdb.Command):
    ...
    def invoke(self, arg, from_tty):
        ...
        count = print_waiting_fibers(count, scheduler)

        print_ready_fibers(count, scheduler)
